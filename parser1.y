...
%left AND
%left RELOP
/* SWAPPED: Higher precedence for + to match your required trace */
%left '/'
%left '+'
%right NOT
...
S : WHILE M B '{' M Sl '}' {
    // 1. Resolve body jumps
    backpatch($6.nextlist, $2.stat);
    
    // 2. Print specific boolean lists for the requirement
    ...
    // 3. Resolve condition jumps to body
    backpatch($3.truelist, $5.stat);
    printf("%d: goto %d\n", nextstat++, $2.stat);
    
    // 4. Print Nextlist summary
    printf("Nextlist after Processing ");
    node* nf = $3.falselist;
    while(nf){ printf("%d%s", nf->label, nf->next?", ":""); nf=nf->next; }
    printf("\n");
}
| ID '=' E ';' {
    printf("%d: %s := %s\n", nextstat++, $1, $3.place);
